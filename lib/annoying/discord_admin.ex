defmodule Annoying.DiscordAdmin do
  ...
  @spec parse_command(List.Char.t()) :: {:ok, command} | {:error, error}
  ...
  defp parse_form({command, _, args}) do
    keywords = case args do
      [kws] when is_list(kws) -> kws
      _ -> []
    end
    case Keyword.fetch(@command, command) do
      :error ->
        similar = Enum.min_by(@command_strings, &String.jaro_distance(Atom.to_string(command), &1))
